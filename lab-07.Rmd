...
ggplot(data = evals, aes(x = score)) +
  ...
  labs(x = "student score", y = NULL)
  
```

the student's score is skewed to the left. This tells me the students are generous and tend give a high score. This is expected as they aren't unbiased and probably want favor with teaching staff.
...
score_bty_fit <- linear_reg() %>%
  set_engine("lm") %>%
  fit(log(score) ~ bty_avg, data = evals)
```
...
1.34612249. 
Yes if the students give a low average beauty rating it makes sense that their overall score might be low and unbiased.
